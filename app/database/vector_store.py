# import logging
# import time
# from typing import Any, List, Optional, Tuple, Union
# from datetime import datetime

# import pandas as pd
# from config.settings import get_settings
# from openai import OpenAI
# from timescale_vector import client


# class VectorStore:
#     """A class for managing vector operations and database interactions."""

#     def __init__(self):
#         """Initialize the VectorStore with settings, OpenAI client, and Timescale Vector client."""
#         self.settings = get_settings()
#         self.openai_client = OpenAI(api_key=self.settings.openai.api_key)
#         self.embedding_model = self.settings.openai.embedding_model
#         self.vector_settings = self.settings.vector_store
#         self.vec_client = client.Sync(
#             self.settings.database.service_url,
#             self.vector_settings.table_name,
#             self.vector_settings.embedding_dimensions,
#             time_partition_interval=self.vector_settings.time_partition_interval,
#         )

#     def get_embedding(self, text: str) -> List[float]:
#         """
#         Generate embedding for the given text.

#         Args:
#             text: The input text to generate an embedding for.

#         Returns:
#             A list of floats representing the embedding.
#         """
#         text = text.replace("\n", " ")
#         start_time = time.time()
#         embedding = (
#             self.openai_client.embeddings.create(
#                 input=[text],
#                 model=self.embedding_model,
#             )
#             .data[0]
#             .embedding
#         )
#         elapsed_time = time.time() - start_time
#         logging.info(f"Embedding generated in {elapsed_time:.3f} seconds")
#         return embedding

#     def create_tables(self) -> None:
#         """Create the necessary tablesin the database"""
#         self.vec_client.create_tables()

#     # def create_index(self) -> None:
#     #     """Create the StreamingDiskANN index to spseed up similarity search"""
#     #     self.vec_client.create_embedding_index(client.DiskAnnIndex())

#     def create_index(self) -> None:
#         """Create the StreamingDiskANN index if it doesn't already exist."""
#         with self.vec_client.connect() as conn:
#             with conn.cursor() as cur:
#                 cur.execute("SELECT to_regclass('embeddings_embedding_idx')")
#                 if not cur.fetchone()[0]:  # If index doesn't exist
#                     self.vec_client.create_embedding_index(client.DiskAnnIndex())
#                     logging.info("Created DiskANN index")
#                 else:
#                     logging.info("DiskANN index already exists")


#     def drop_index(self) -> None:
#         """Drop the StreamingDiskANN index in the database"""
#         self.vec_client.drop_embedding_index()

#     def upsert(self, df: pd.DataFrame) -> None:
#         """
#         Insert or update records in the database from a pandas DataFrame.

#         Args:
#             df: A pandas DataFrame containing the data to insert or update.
#                 Expected columns: id, metadata, contents, embedding
#         """
#         records = df.to_records(index=False)
#         self.vec_client.upsert(list(records))
#         logging.info(
#             f"Inserted {len(df)} records into {self.vector_settings.table_name}"
#         )

#     def search(
#         self,
#         query_text: str,
#         limit: int = 5,
#         metadata_filter: Union[dict, List[dict]] = None,
#         predicates: Optional[client.Predicates] = None,
#         time_range: Optional[Tuple[datetime, datetime]] = None,
#         return_dataframe: bool = True,
#     ) -> Union[List[Tuple[Any, ...]], pd.DataFrame]:
#         """
#         Query the vector database for similar embeddings based on input text.

#         More info:
#             https://github.com/timescale/docs/blob/latest/ai/python-interface-for-pgvector-and-timescale-vector.md

#         Args:
#             query_text: The input text to search for.
#             limit: The maximum number of results to return.
#             metadata_filter: A dictionary or list of dictionaries for equality-based metadata filtering.
#             predicates: A Predicates object for complex metadata filtering.
#                 - Predicates objects are defined by the name of the metadata key, an operator, and a value.
#                 - Operators: ==, !=, >, >=, <, <=
#                 - & is used to combine multiple predicates with AND operator.
#                 - | is used to combine multiple predicates with OR operator.
#             time_range: A tuple of (start_date, end_date) to filter results by time.
#             return_dataframe: Whether to return results as a DataFrame (default: True).

#         Returns:
#             Either a list of tuples or a pandas DataFrame containing the search results.

#         Basic Examples:
#             Basic search:
#                 vector_store.search("What are your shipping options?")
#             Search with metadata filter:
#                 vector_store.search("Shipping options", metadata_filter={"category": "Shipping"})

#         Predicates Examples:
#             Search with predicates:
#                 vector_store.search("Pricing", predicates=client.Predicates("price", ">", 100))
#             Search with complex combined predicates:
#                 complex_pred = (client.Predicates("category", "==", "Electronics") & client.Predicates("price", "<", 1000)) | \
#                                (client.Predicates("category", "==", "Books") & client.Predicates("rating", ">=", 4.5))
#                 vector_store.search("High-quality products", predicates=complex_pred)

#         Time-based filtering:
#             Search with time range:
#                 vector_store.search("Recent updates", time_range=(datetime(2024, 1, 1), datetime(2024, 1, 31)))
#         """
#         query_embedding = self.get_embedding(query_text)

#         start_time = time.time()

#         search_args = {
#             "limit": limit,
#         }

#         if metadata_filter:
#             search_args["filter"] = metadata_filter

#         if predicates:
#             search_args["predicates"] = predicates

#         if time_range:
#             start_date, end_date = time_range
#             search_args["uuid_time_filter"] = client.UUIDTimeRange(start_date, end_date)

#         results = self.vec_client.search(query_embedding, **search_args)
#         elapsed_time = time.time() - start_time

#         logging.info(f"Vector search completed in {elapsed_time:.3f} seconds")

#         if return_dataframe:
#             return self._create_dataframe_from_results(results)
#         else:
#             return results

#     def _create_dataframe_from_results(
#         self,
#         results: List[Tuple[Any, ...]],
#     ) -> pd.DataFrame:
#         """
#         Create a pandas DataFrame from the search results.

#         Args:
#             results: A list of tuples containing the search results.

#         Returns:
#             A pandas DataFrame containing the formatted search results.
#         """
#         # Convert results to DataFrame
#         df = pd.DataFrame(
#             results, columns=["id", "metadata", "content", "embedding", "distance"]
#         )

#         # Expand metadata column
#         df = pd.concat(
#             [df.drop(["metadata"], axis=1), df["metadata"].apply(pd.Series)], axis=1
#         )

#         # Convert id to string for better readability
#         df["id"] = df["id"].astype(str)

#         return df

#     def delete(
#         self,
#         ids: List[str] = None,
#         metadata_filter: dict = None,
#         delete_all: bool = False,
#     ) -> None:
#         """Delete records from the vector database.

#         Args:
#             ids (List[str], optional): A list of record IDs to delete.
#             metadata_filter (dict, optional): A dictionary of metadata key-value pairs to filter records for deletion.
#             delete_all (bool, optional): A boolean flag to delete all records.

#         Raises:
#             ValueError: If no deletion criteria are provided or if multiple criteria are provided.

#         Examples:
#             Delete by IDs:
#                 vector_store.delete(ids=["8ab544ae-766a-11ef-81cb-decf757b836d"])

#             Delete by metadata filter:
#                 vector_store.delete(metadata_filter={"category": "Shipping"})

#             Delete all records:
#                 vector_store.delete(delete_all=True)
#         """
#         if sum(bool(x) for x in (ids, metadata_filter, delete_all)) != 1:
#             raise ValueError(
#                 "Provide exactly one of: ids, metadata_filter, or delete_all"
#             )

#         if delete_all:
#             self.vec_client.delete_all()
#             logging.info(f"Deleted all records from {self.vector_settings.table_name}")
#         elif ids:
#             self.vec_client.delete_by_ids(ids)
#             logging.info(
#                 f"Deleted {len(ids)} records from {self.vector_settings.table_name}"
#             )
#         elif metadata_filter:
#             self.vec_client.delete_by_metadata(metadata_filter)
#             logging.info(
#                 f"Deleted records matching metadata filter from {self.vector_settings.table_name}"
#             )
# -----------------------------------------------------------------------------
# import logging
# import time
# from typing import Any, List, Optional, Tuple, Union
# from datetime import datetime

# import pandas as pd
# from config.settings import get_settings
# from openai import OpenAI
# from timescale_vector import client


# class VectorStore:
#     """A class for managing vector operations and database interactions."""

#     def __init__(self):
#         """Initialize the VectorStore with settings, OpenAI client, and Timescale Vector client."""
#         self.settings = get_settings()
#         self.openai_client = OpenAI(api_key=self.settings.openai.api_key)
#         self.embedding_model = self.settings.openai.embedding_model
#         self.vector_settings = self.settings.vector_store
#         self.vec_client = client.Sync(
#             self.settings.database.service_url,
#             self.vector_settings.table_name,
#             self.vector_settings.embedding_dimensions,
#             time_partition_interval=self.vector_settings.time_partition_interval,
#         )

#     def get_embedding(self, text: str) -> List[float]:
#         """
#         Generate embedding for the given text.

#         Args:
#             text: The input text to generate an embedding for.

#         Returns:
#             A list of floats representing the embedding.
#         """
#         text = text.replace("\n", " ")
#         start_time = time.time()
#         embedding = (
#             self.openai_client.embeddings.create(
#                 input=[text],
#                 model=self.embedding_model,
#             )
#             .data[0]
#             .embedding
#         )
#         elapsed_time = time.time() - start_time
#         logging.info(f"Embedding generated in {elapsed_time:.3f} seconds")
#         return embedding

#     def create_tables(self) -> None:
#         """Create the necessary tables in the database."""
#         try:
#             self.vec_client.create_tables()
#             logging.info(f"Created tables for {self.vector_settings.table_name}")
#         except Exception as e:
#             logging.warning(f"Tables may already exist: {str(e)}")

#     def create_index(self) -> None:
#         """Create the StreamingDiskANN index if it doesn't already exist."""
#         with self.vec_client.connect() as conn:
#             with conn.cursor() as cur:
#                 cur.execute("SELECT to_regclass('embeddings_embedding_idx')")
#                 if not cur.fetchone()[0]:  # If index doesn't exist
#                     self.vec_client.create_embedding_index(client.DiskAnnIndex())
#                     logging.info("Created DiskANN index")
#                 else:
#                     logging.info("DiskANN index already exists")

#     def drop_index(self) -> None:
#         """Drop the StreamingDiskANN index if it exists."""
#         try:
#             self.vec_client.drop_embedding_index()
#             logging.info("Dropped DiskANN index")
#         except Exception as e:
#             logging.warning(f"Index may not exist: {str(e)}")

#     def upsert(self, df: pd.DataFrame) -> None:
#         """
#         Insert or update records in the database from a pandas DataFrame.

#         Args:
#             df: A pandas DataFrame containing the data to insert or update.
#                 Expected columns: id, metadata, contents, embedding
#         """
#         try:
#             records = df.to_records(index=False)
#             self.vec_client.upsert(list(records))
#             logging.info(
#                 f"Successfully upserted {len(df)} records into {self.vector_settings.table_name}"
#             )
#         except Exception as e:
#             logging.error(f"Failed to upsert records: {str(e)}")
#             raise

#     def search(
#         self,
#         query_text: str,
#         limit: int = 5,
#         metadata_filter: Union[dict, List[dict]] = None,
#         predicates: Optional[client.Predicates] = None,
#         time_range: Optional[Tuple[datetime, datetime]] = None,
#         return_dataframe: bool = True,
#     ) -> Union[List[Tuple[Any, ...]], pd.DataFrame]:
#         """
#         Query the vector database for similar embeddings based on input text.
#         """
#         try:
#             query_embedding = self.get_embedding(query_text)
#             start_time = time.time()

#             search_args = {
#                 "limit": limit,
#                 "filter": metadata_filter,
#                 "predicates": predicates,
#             }

#             if time_range:
#                 start_date, end_date = time_range
#                 search_args["uuid_time_filter"] = client.UUIDTimeRange(start_date, end_date)

#             results = self.vec_client.search(query_embedding, **search_args)
#             elapsed_time = time.time() - start_time

#             logging.info(f"Vector search completed in {elapsed_time:.3f} seconds")

#             if return_dataframe:
#                 return self._create_dataframe_from_results(results)
#             return results
#         except Exception as e:
#             logging.error(f"Search failed: {str(e)}")
#             raise

#     def _create_dataframe_from_results(
#         self,
#         results: List[Tuple[Any, ...]],
#     ) -> pd.DataFrame:
#         """
#         Create a pandas DataFrame from the search results.
#         """
#         try:
#             df = pd.DataFrame(
#                 results, columns=["id", "metadata", "content", "embedding", "distance"]
#             )
#             df = pd.concat(
#                 [df.drop(["metadata"], axis=1), df["metadata"].apply(pd.Series)], axis=1
#             )
#             df["id"] = df["id"].astype(str)
#             return df
#         except Exception as e:
#             logging.error(f"Failed to create DataFrame from results: {str(e)}")
#             raise

#     def delete(
#         self,
#         ids: List[str] = None,
#         metadata_filter: dict = None,
#         delete_all: bool = False,
#     ) -> None:
#         """Delete records from the vector database."""
#         try:
#             if sum(bool(x) for x in (ids, metadata_filter, delete_all)) != 1:
#                 raise ValueError(
#                     "Provide exactly one of: ids, metadata_filter, or delete_all"
#                 )

#             if delete_all:
#                 self.vec_client.delete_all()
#                 logging.info(f"Deleted all records from {self.vector_settings.table_name}")
#             elif ids:
#                 self.vec_client.delete_by_ids(ids)
#                 logging.info(f"Deleted {len(ids)} records")
#             elif metadata_filter:
#                 self.vec_client.delete_by_metadata(metadata_filter)
#                 logging.info(f"Deleted records matching metadata filter")
#         except Exception as e:
#             logging.error(f"Deletion failed: {str(e)}")
#             raise


import logging
import time
from typing import Any, List, Optional, Tuple, Union
from datetime import datetime

import pandas as pd
from config.settings import get_settings
from openai import OpenAI
from timescale_vector import client


class VectorStore:
    """A class for managing vector operations and database interactions with enhanced duplicate handling."""

    def __init__(self):
        """Initialize the VectorStore with settings, OpenAI client, and Timescale Vector client."""
        self.settings = get_settings()
        self.openai_client = OpenAI(api_key=self.settings.openai.api_key)
        self.embedding_model = self.settings.openai.embedding_model
        self.vector_settings = self.settings.vector_store
        self.vec_client = client.Sync(
            self.settings.database.service_url,
            self.vector_settings.table_name,
            self.vector_settings.embedding_dimensions,
            time_partition_interval=self.vector_settings.time_partition_interval,
        )

    def get_embedding(self, text: str) -> List[float]:
        """
        Generate embedding for the given text with error handling.

        Args:
            text: The input text to generate an embedding for.

        Returns:
            A list of floats representing the embedding.

        Raises:
            Exception: If embedding generation fails.
        """
        try:
            text = text.replace("\n", " ")
            start_time = time.time()
            embedding = (
                self.openai_client.embeddings.create(
                    input=[text],
                    model=self.embedding_model,
                )
                .data[0]
                .embedding
            )
            elapsed_time = time.time() - start_time
            logging.info(f"Embedding generated in {elapsed_time:.3f} seconds")
            return embedding
        except Exception as e:
            logging.error(f"Failed to generate embedding: {str(e)}")
            raise

    def create_tables(self) -> None:
        """Create the necessary tables in the database with error handling."""
        try:
            self.vec_client.create_tables()
            logging.info(f"Created tables for {self.vector_settings.table_name}")
        except Exception as e:
            logging.warning(f"Tables may already exist: {str(e)}")

    def create_index(self) -> None:
        """Create the StreamingDiskANN index if it doesn't already exist."""
        try:
            with self.vec_client.connect() as conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT to_regclass('embeddings_embedding_idx')")
                    if not cur.fetchone()[0]:
                        self.vec_client.create_embedding_index(client.DiskAnnIndex())
                        logging.info("Created DiskANN index")
                    else:
                        logging.info("DiskANN index already exists")
        except Exception as e:
            logging.error(f"Index creation failed: {str(e)}")
            raise

    def drop_index(self) -> None:
        """Drop the StreamingDiskANN index if it exists."""
        try:
            self.vec_client.drop_embedding_index()
            logging.info("Dropped DiskANN index")
        except Exception as e:
            logging.warning(f"Index may not exist: {str(e)}")

    def upsert(self, df: pd.DataFrame, check_duplicates: bool = True) -> None:
        """
        Enhanced upsert with duplicate checking.

        Args:
            df: DataFrame with columns: id, metadata, content, embedding
            check_duplicates: If True, checks for existing content before upserting

        Raises:
            Exception: If upsert operation fails
        """
        try:
            if check_duplicates:
                # Get existing content to check against
                existing_contents = set()
                with self.vec_client.connect() as conn:
                    existing_contents.update(pd.read_sql(
                        f"SELECT content FROM {self.vector_settings.table_name}",
                        conn
                    )['content'])

                # Filter out duplicates
                original_count = len(df)
                df = df[~df['content'].isin(existing_contents)]
                filtered_count = len(df)

                if filtered_count < original_count:
                    logging.info(f"Filtered out {original_count - filtered_count} duplicates")

            # Proceed with upsert
            records = df.to_records(index=False)
            self.vec_client.upsert(list(records))
            logging.info(f"Upserted {len(df)} records")

        except Exception as e:
            logging.error(f"Upsert failed: {str(e)}")
            raise

    def find_exact_duplicates(self) -> pd.DataFrame:
        """
        Find records with identical embeddings.

        Returns:
            DataFrame with duplicate records and their count
        """
        with self.vec_client.connect() as conn:
            query = f"""
            SELECT id, content, embedding, COUNT(*) OVER (PARTITION BY embedding) as dup_count
            FROM {self.vector_settings.table_name}
            WHERE embedding IN (
                SELECT embedding
                FROM {self.vector_settings.table_name}
                GROUP BY embedding
                HAVING COUNT(*) > 1
            )
            ORDER BY embedding
            """
            return pd.read_sql(query, conn)

    def find_near_duplicates(self, similarity_threshold: float = 0.98) -> pd.DataFrame:
        """
        Find records with similar embeddings using cosine similarity.

        Args:
            similarity_threshold: Min similarity score (0-1) to consider as duplicates

        Returns:
            DataFrame of similar record pairs and their similarity scores
        """
        with self.vec_client.connect() as conn:
            query = f"""
            WITH embeddings AS (
                SELECT id, content, embedding
                FROM {self.vector_settings.table_name}
            )
            SELECT a.id as id1, b.id as id2,
                   a.content as content1, b.content as content2,
                   (1 - (a.embedding <=> b.embedding)) as similarity
            FROM embeddings a
            JOIN embeddings b ON a.id < b.id
            WHERE (1 - (a.embedding <=> b.embedding)) > {similarity_threshold}
            ORDER BY similarity DESC
            """
            return pd.read_sql(query, conn)

    def clean_duplicates(self, similarity_threshold: float = 0.98) -> dict:
        """
        Remove duplicates and return cleanup report.

        Args:
            similarity_threshold: Min similarity score for near-duplicates

        Returns:
            Dictionary with cleanup statistics
        """
        results = {
            'exact_duplicates': self.find_exact_duplicates(),
            'near_duplicates': self.find_near_duplicates(similarity_threshold)
        }

        # Remove exact duplicates
        if not results['exact_duplicates'].empty:
            with self.vec_client.connect() as conn:
                query = f"""
                DELETE FROM {self.vector_settings.table_name}
                WHERE id IN (
                    SELECT id FROM (
                        SELECT id,
                               ROW_NUMBER() OVER (PARTITION BY embedding ORDER BY id DESC) as rn
                        FROM {self.vector_settings.table_name}
                    ) t WHERE t.rn > 1
                )
                """
                with conn.cursor() as cur:
                    cur.execute(query)
                    results['exact_deleted'] = cur.rowcount
                    conn.commit()

        # Remove near duplicates
        if not results['near_duplicates'].empty:
            with self.vec_client.connect() as conn:
                query = f"""
                DELETE FROM {self.vector_settings.table_name}
                WHERE id IN %s
                """
                with conn.cursor() as cur:
                    dup_ids = tuple(set(results['near_duplicates']['id2']))
                    cur.execute(query, (dup_ids,))
                    results['near_deleted'] = cur.rowcount
                    conn.commit()

        return results

    def search(
        self,
        query_text: str,
        limit: int = 5,
        metadata_filter: Union[dict, List[dict]] = None,
        predicates: Optional[client.Predicates] = None,
        time_range: Optional[Tuple[datetime, datetime]] = None,
        return_dataframe: bool = True,
    ) -> Union[List[Tuple[Any, ...]], pd.DataFrame]:
        """
        Enhanced search with better error handling.

        Args:
            query_text: Text to search for
            limit: Max number of results
            metadata_filter: Filter by metadata
            predicates: Complex filter conditions
            time_range: Filter by time range
            return_dataframe: Return as DataFrame if True

        Returns:
            Search results in requested format
        """
        try:
            query_embedding = self.get_embedding(query_text)
            start_time = time.time()

            search_args = {
                "limit": limit,
                "filter": metadata_filter,
                "predicates": predicates,
            }

            if time_range:
                start_date, end_date = time_range
                search_args["uuid_time_filter"] = client.UUIDTimeRange(start_date, end_date)

            results = self.vec_client.search(query_embedding, **search_args)
            elapsed_time = time.time() - start_time

            logging.info(f"Search completed in {elapsed_time:.3f} seconds")

            if return_dataframe:
                return self._create_dataframe_from_results(results)
            return results

        except Exception as e:
            logging.error(f"Search failed: {str(e)}")
            raise

    def _create_dataframe_from_results(self, results: List[Tuple[Any, ...]]) -> pd.DataFrame:
        """Convert search results to DataFrame with expanded metadata."""
        try:
            df = pd.DataFrame(
                results, columns=["id", "metadata", "content", "embedding", "distance"]
            )
            df = pd.concat(
                [df.drop(["metadata"], axis=1), df["metadata"].apply(pd.Series)], axis=1
            )
            df["id"] = df["id"].astype(str)
            return df
        except Exception as e:
            logging.error(f"Failed to create DataFrame: {str(e)}")
            raise

    def delete(
        self,
        ids: List[str] = None,
        metadata_filter: dict = None,
        delete_all: bool = False,
    ) -> None:
        """
        Enhanced delete with better error handling.

        Args:
            ids: List of IDs to delete
            metadata_filter: Metadata filter for deletion
            delete_all: Delete all records if True

        Raises:
            ValueError: If invalid combination of parameters
        """
        try:
            if sum(bool(x) for x in (ids, metadata_filter, delete_all)) != 1:
                raise ValueError("Provide exactly one of: ids, metadata_filter, or delete_all")

            if delete_all:
                self.vec_client.delete_all()
                logging.info(f"Deleted all records")
            elif ids:
                self.vec_client.delete_by_ids(ids)
                logging.info(f"Deleted {len(ids)} records")
            elif metadata_filter:
                self.vec_client.delete_by_metadata(metadata_filter)
                logging.info(f"Deleted records matching metadata filter")

        except Exception as e:
            logging.error(f"Deletion failed: {str(e)}")
            raise